...
from ..translation_terms.opposing_account_patterns import extract_opposing_account
...
class YamlTarget(Target):
    ...
    def export_transactions(
        ...
        try:
            ...
            yaml_data = self._transactions_to_yaml_data(transactions, config)
            ...
            self._generate_summary(summary_path, transactions, config)
            ...
            return {
                ...
                "metadata": {
                    ...
                    if export_type != "all_consolidated"
                    and export_type != "bank_consolidated"
                    else "all_accounts",
    ...
    def _determine_opposing_account(
        self, transaction: Transaction, description: str
    ) -> tuple[str, str]:
        """
        ...
        if (
            transaction.channel
            and transaction.channel.upper() in ["ATM", "CDM"]
        ...
        if (
            ...
            and transaction.category.upper() == "BRANCH"
    ...
    def _remove_none_values(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                if key in ["translated", "original", "formatted"] and value == "":
                    result[key] = value
                elif value != "":
                    result[key] = value
    ...
    def _get_top_transaction(
        self, transactions: list[Transaction], which: str
    ) -> dict[str, Any]:
    ...
    def _get_recent_transactions(
        self, transactions: list[Transaction], count: int
    ) -> list[dict[str, Any]]:

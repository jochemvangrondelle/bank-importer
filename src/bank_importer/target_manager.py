    ...
    def export_all_files_and_consolidated(
        ...
        for source_file in source_files:
            ...
            if transactions:
                ...
                session_name = f"export_{target_name}_source_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
                export_session_id = self.db_manager.create_export_session(
                    session_name,
                    target_name,
        ...
        if all_transactions:
            ...
            session_name = f"export_{target_name}_all_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
            export_session_id = self.db_manager.create_export_session(
                session_name,
                target_name,
                "all_transactions",

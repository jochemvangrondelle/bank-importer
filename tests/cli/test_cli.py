...
from bank_importer.cli.main import app
...
class TestCLI:
    ...
    def test_cli_run_without_config(self, runner: CliRunner) -> None:
        ...
        with (
            patch("bank_importer.cli.commands.status.status") as mock_status,
            patch(
                "bank_importer.cli.commands.import_files.import_files",
            ) as mock_import,
            patch(
                "bank_importer.cli.commands.export_multi.export_multi",
            ) as mock_export,
        ):
...
""")
    ...
    def test_cli_init(self, runner: CliRunner, tmp_path: Path) -> None:
        ...
        with patch("bank_importer.cli.commands.init.init"):
    ...
    def test_cli_error_handling(self, runner: CliRunner) -> None:
        ...
        with patch("bank_importer.cli.commands.status.status") as mock_status:
    ...
    def test_account_parameter_validation_valid_account(
        ...
        with (
            ...
            result = runner.invoke(
                app,
                ["run", "--account", "krungsri_pdf", "--dry-run"],
            )
            assert result.exit_code == 0
            mock_status.assert_called()
            mock_import.assert_called()

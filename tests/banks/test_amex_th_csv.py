...
class TestAmexThCsvParser:
    ...
    def test_parse_file_success(
        ...
        payment_txn = next(t for t in transactions if "NET PAYMENT" in t.description)
    ...
    def test_amount_normalization(
        ...
        credit_transactions = [
            t for t in transactions if "NET PAYMENT" in t.description
        ]
    ...
    def test_transaction_type_detection(self, parser: AmexThCsvParser) -> None:
        ...
        assert (
            parser._determine_transaction_type("SHOPEETH", Decimal(-100)) == "purchase"
        )

...
class TestKrungsriPdfParser(BaseParserTest):
    ...
    def test_parse_real_krungsri_pdf(
        self,
        test_data_dir: Path,
        sample_account_config_with_password: dict,
    ) -> None:
        ...
        with patch("pypdfium2.PdfDocument") as mock_pdf_document:
            mock_pdf = Mock()
            mock_pdf.__len__ = Mock(return_value=1)
            mock_pdf.close = Mock()
            mock_page = Mock()
            mock_textpage = Mock()
            mock_textpage.get_text_range.return_value = mock_pdf_content
            ...
            transactions = self.get_transactions_from_parser(
                KrungsriPdfParser,
                pdf_file,
                sample_account_config_with_password,
            )
    ...
    def test_timezone_aware_timestamps(
        ...
        for transaction in transactions:
            ...
            assert bangkok_time.hour >= 0
            assert bangkok_time.hour <= 23
            assert bangkok_time.minute >= 0
            assert bangkok_time.minute <= 59
            assert bangkok_time.second >= 0
            assert bangkok_time.second <= 59

...
The translation terms system has been moved to a dedicated directory structure (`src/bank_importer_th/translation_terms/`) to provide better organization and extensibility for multiple languages. Currently, it includes comprehensive Thai financial terms extracted from actual transaction data.

## Directory Structure

```
src/bank_importer_th/translation_terms/
...
Edit `src/bank_importer_th/translation_terms/thai_terms.py`:
...
1. Create a new file: `src/bank_importer_th/translation_terms/english_terms.py`
...
```python
from bank_importer_th.translation_service import translate_description
